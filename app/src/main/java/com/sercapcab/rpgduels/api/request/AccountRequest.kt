...
suspend fun getCharactersFromAccount(username: String, authCredentials: Pair<String, String>): List<CharacterData>? {
    return withContext(Dispatchers.IO) {
        try {
            val retrofit = RetrofitSingleton.getRetrofitInstance()
            val service = retrofit?.create(AccountAPIService::class.java)
            val authentication = Credentials.basic(authCredentials.first, authCredentials.second)
            val call = service?.getCharactersFromAccount(username = username, authHeader = authentication)
            val response = call?.execute()
